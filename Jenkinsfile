...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 4157
        NAMESPACE = "snapflow-cc"
        REGISTRY_HOSTNAME = "chamface"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "snapflow-frontend"
        DEPLOYMENT_NAME = "snapflow-frontend-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'snapflow-cc') {
